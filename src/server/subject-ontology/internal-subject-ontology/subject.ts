...
export class Subject {
  ...
  getChildren() {
    return [...this.childSubjects].map(subject => subject.name)
  }

  getComponentSets() {
    return [...this.componentSubjectSets].map(set => [...set].map(subject => subject.name))
  }

  addComponentSet(componentSet: SetOfSubjects) {
  ...
  removeComponentSet(componentSet: SetOfSubjects) {
  ...
  search(key: string, childrenOnly: boolean = false) {
      ...
      if (!subjectReachability?.reached) {
        componentSets.forEach(setOfSubjects => {
          if (!!Reachability.reachabilityFromSet(key, setOfSubjects)) {
            if (subject.getOrCreateReachability(key).addReachableComponentSet(setOfSubjects)) {
   ...
   canDestroy() {
    return this.childSubjects.size === 0 &&
      this.parentSubjects.size === 0 &&
      this.componentSubjectSets.size === 0 &&
      this.componentSubjectParents.size === 0
  }
