  ...
  getFromStore$() {
    return voidObs$.pipe(
      ...
      map(creds => creds.map(cred => {
        const data = JSON.parse(cred.attrs!['proposal']) as Schemas.MasterProposalStateSchema
        const votes = new Map(Object.entries(data.proposal.votes))
        const proposal: Server.ControllerMasterProposal = {
          proposalType: data.proposal.action,
          subject: data.proposal.subject,
          did: data.proposal.did,
          votes
        }
